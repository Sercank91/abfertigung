...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (transitOfficeIds && Array.isArray(transitOfficeIds)) {
      ...
      await pool.query('DELETE FROM "RouteTransitOffice" WHERE "routeId" = $1', [routeId])

      // Erstelle neue Transit-Offices
      if (transitOfficeIds.length > 0) {
        const transitValues = transitOfficeIds
          .map(
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await pool.query('UPDATE "Route" SET "isActive" = false, "updatedAt" = NOW() WHERE id = $1', [
      routeId,
    ])
