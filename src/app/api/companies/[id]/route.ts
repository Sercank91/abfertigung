...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ error: 'Fehler beim Abrufen der Firma' }, { status: 500 })
  }
}
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { name, country, address, postalCode, city, emails, phones, guaranteeIds, isActive } =
      body
    ...
    if (!country || !country.trim()) {
      return NextResponse.json({ error: 'Land ist erforderlich' }, { status: 400 })
    }
    ...
    try {
      ...
      await client.query('DELETE FROM "CompanyGuarantee" WHERE "companyId" = $1', [params.id])
    ...
    return NextResponse.json({ error: 'Fehler beim Aktualisieren der Firma' }, { status: 500 })
  }
}
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await pool.query('DELETE FROM "Company" WHERE id = $1 AND "tenantId" = $2', [
      params.id,
      user.tenantId,
    ])
    ...
    return NextResponse.json({ error: 'Fehler beim Löschen der Firma' }, { status: 500 })
  }
}
