...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const checkUser = await pool.query('SELECT id FROM "User" WHERE id = $1 AND "tenantId" = $2', [
      params.id,
      user.tenantId,
    ])
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await pool.query('UPDATE "User" SET "isActive" = false, "updatedAt" = NOW() WHERE id = $1', [
      params.id,
    ])
