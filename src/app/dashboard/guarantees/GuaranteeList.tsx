...
export default function GuaranteeList({ initialGuarantees, canEdit, userRole }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      const url = editingGuarantee ? `/api/guarantees/${editingGuarantee.id}` : '/api/guarantees'
      ...
      if (editingGuarantee) {
        setGuarantees(
          guarantees.map((g) =>
            g.id === editingGuarantee.id ? { ...data, _count: g._count || { companies: 0 } } : g
          )
        )
      } else {
        setGuarantees([
      ...
      <div className="bg-white mb-4 p-3 border border-[#c6c6c6] rounded-sm">
        <div className="flex items-center gap-3">
          ...
          <div className="text-xs text-[#525252] whitespace-nowrap">
            ...
            {filteredGuarantees.length === 1 ? 'Bürgschaft' : 'Bürgschaften'}
          </div>
                  ...
                  >
                    ...
                    <td className="px-3 py-2">
                      <div className="text-sm text-[#525252]">
                        {guarantee._count?.companies || 0}{' '}
                        {guarantee._count?.companies === 1 ? 'Firma' : 'Firmen'}
                      </div>
                    </td>
                    <td className="px-3 py-2">
                      <span
